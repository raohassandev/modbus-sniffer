...
class TcpDiscoveryClient{
  constructor({host,port=502,timeoutMs=750,signal=null,onEvidence=null}={}){this.host=String(host||'').trim();this.port=Number(port);this.timeoutMs=Math.max(100,Math.min(10000,Number(timeoutMs)||750));this.signal=signal;this.onEvidence=typeof onEvidence==='function'?onEvidence:null;this.connectionId=`discovery:tcp:${this.host}:${this.port}`;this.socket=null;this.parser=new ModbusTcpStreamParser();this.pending=new Map();this.tid=0;this.closed=false;this.parser.on('frame',(frame,ts)=>this._frame(frame,ts));}
  async connect(){
  ...
  _frame(frame,ts){const p=this.pending.get(frame.transactionId);if(!p||Number(frame.unitId)!==p.unitId)return;const rttMs=ts-p.sentAt;this.pending.delete(frame.transactionId);clearTimeout(p.timer);p.cleanup?.();this._evidence({type:'traffic.rx',timestamp:ts,unitId:Number(frame.unitId),functionCode:Number(frame.functionCode||43),raw:frame.raw,transactionId:frame.transactionId,rttMs});p.resolve({frame,rttMs});}
  query(unitId,code=1,objectId=0){
    ...
    this._evidence({type:'traffic.tx',timestamp:sentAt,unitId:Number(unitId),functionCode:43,raw:request,transactionId:tid,readDeviceIdCode:code,objectId});
    return new Promise((resolve,reject)=>{let settled=false;const finish=(err,value)=>{if(settled)return;settled=true;this.pending.delete(tid);clearTimeout(timer);cleanup();err?reject(err):resolve(value);};const onAbort=()=>finish(abortError());const cleanup=()=>this.signal?.removeEventListener?.('abort',onAbort);const timer=setTimeout(()=>{this._evidence({type:'discovery.timeout',timestamp:Date.now(),unitId:Number(unitId),functionCode:43,raw:request,transactionId:tid,timeoutMs:this.timeoutMs});finish(Object.assign(new Error(`No response from Unit ${unitId} after ${this.timeoutMs} ms.`),{code:'DISCOVERY_TIMEOUT'}));},this.timeoutMs);this.signal?.addEventListener?.('abort',onAbort,{once:true});this.pending.set(tid,{unitId:Number(unitId),sentAt,timer,cleanup,request,resolve:value=>finish(null,value),reject:err=>finish(err)});this.socket.write(request,e=>{if(e)finish(e);});});
  }
  _evidence({type,timestamp,unitId,functionCode,raw,transactionId=null,rttMs=null,timeoutMs=null,readDeviceIdCode=null,objectId=null}){this.onEvidence?.({timestamp,type,source:'active-discovery',ownerMode:'discovery',connectionId:this.connectionId,unitId,functionCode,rawHex:Buffer.from(raw||[]).toString('hex').toUpperCase(),details:{framing:'tcp',transport:'TCP',transactionId,rttMs,timeoutMs,readDeviceIdCode,objectId}});}
...
async function scanTcpDeviceIds({host,port=502,unitStart=1,unitEnd=247,timeoutMs=750,interRequestMs=75,readDeviceIdCode=1,maxSegments=8,signal=null,onProgress=null,onEvidence=null}={}){
  const start=clampUnit(unitStart,0,255),end=clampUnit(unitEnd,0,255);if(end<start)throw new Error('unitEnd must be >= unitStart.');
  const client=await new TcpDiscoveryClient({host,port,timeoutMs,signal,onEvidence}).connect(),results=[];
...
async function scanRtuDeviceIds({port,baudRate=9600,dataBits=8,parity='none',stopBits=1,unitStart=1,unitEnd=247,timeoutMs=500,interRequestMs=100,readDeviceIdCode=1,maxSegments=8,maintenanceConfirmed=false,exclusiveBusConfirmed=false,signal=null,onProgress=null,onEvidence=null}={}){
  ...
  const connectionId=`discovery:rtu:${String(port||'')}`;
  const emitEvidence=({type,timestamp,unitId,raw,rttMs=null,timeoutMs=null})=>onEvidence?.({timestamp,type,source:'active-discovery',ownerMode:'discovery',connectionId,unitId,functionCode:43,rawHex:Buffer.from(raw||[]).toString('hex').toUpperCase(),details:{framing:'rtu',transport:'RTU',rttMs,timeoutMs}});
  extractor.on('frame',(raw,ts)=>{if(!waiter)return;const d=decodeFrame(raw);if(Number(d.slaveId)!==waiter.unitId||Number(d.functionCode)!==43||d.kind==='request')return;const w=waiter;const rttMs=ts-w.sentAt;waiter=null;emitEvidence({type:'traffic.rx',timestamp:ts,unitId:w.unitId,raw,rttMs});w.finish(null,{decoded:d,frame:{decoded:d},raw,rttMs});});
  ...
  const query=(unitId,code,objectId)=>new Promise((resolve,reject)=>{checkAbort(signal);const sentAt=Date.now();const req=buildRtuDeviceIdRequest(unitId,code,objectId);emitEvidence({type:'traffic.tx',timestamp:sentAt,unitId:Number(unitId),raw:req});let settled=false;const finish=(err,value)=>{if(settled)return;settled=true;clearTimeout(timer);signal?.removeEventListener?.('abort',onAbort);if(waiter?.unitId===unitId)waiter=null;err?reject(err):resolve(value);};const onAbort=()=>finish(abortError());const timeout=Math.max(100,Number(timeoutMs)||500);const timer=setTimeout(()=>{emitEvidence({type:'discovery.timeout',timestamp:Date.now(),unitId:Number(unitId),raw:req,timeoutMs:timeout});finish(Object.assign(new Error(`No response from Slave ${unitId} after ${timeoutMs} ms.`),{code:'DISCOVERY_TIMEOUT'}));},timeout);signal?.addEventListener?.('abort',onAbort,{once:true});waiter={unitId:Number(unitId),sentAt,finish};serial.write(req,e=>{if(e){finish(e);return;}serial.drain(e=>{if(e)finish(e);});});});
  const results=[];
