...
function decodeTcpAdu(raw){
  if(!Buffer.isBuffer(raw)||raw.length<8)throw new Error('Invalid Modbus TCP ADU.');
  const transactionId=raw.readUInt16BE(0),protocolId=raw.readUInt16BE(2),length=raw.readUInt16BE(4),unitId=raw[6];
  if(protocolId!==0)throw new Error('Not Modbus TCP (protocol ID is not 0).');
  if(length<2||length>254)throw new Error('Invalid MBAP length field.');
  if(raw.length!==6+length)throw new Error('Invalid MBAP length.');
...
class ModbusTcpStreamParser extends EventEmitter{
  ...
  push(chunk,timestamp=Date.now()){
    ...
    while(this.buffer.length>=7){
      const protocolId=this.buffer.readUInt16BE(2),length=this.buffer.readUInt16BE(4),total=6+length;
      if(protocolId!==0||length<2||length>254){this.emit('noise',this.buffer.subarray(0,1));this.buffer=this.buffer.subarray(1);continue;}
      if(this.buffer.length<total)break;
      const raw=Buffer.from(this.buffer.subarray(0,total));this.buffer=this.buffer.subarray(total);
      try{this.emit('frame',decodeTcpAdu(raw),timestamp);}catch(err){this.emit('error-frame',err,raw);}
    }
  }
