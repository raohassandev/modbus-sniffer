  ...
  _createRuntime(project, signature) {
    ...
    for (const definition of project.charts || []) {
      ...
      for (const series of chart.series) {
        charts.addSeries(chart.documentId, { ...series, source: { sourceKey: series.sourceKey } });
      }
    }

    const projectId = safeProjectId(project.id);
    const logger = new RotatingJsonlLogger({
      directory: path.join(this.dataDir, 'v8-logs', projectId),
      prefix: projectId,
    ...
    for (const definition of project.loggerProfiles || []) {
      ...
      logger.addStream({
        ...
        source: { sourceKey: profile.sourceKey },
        mode: profile.mode,
    ...
    if (historianProfiles.length && sqliteAvailable()) {
      historian = new SqliteHistorian({
        filePath: path.join(this.dataDir, 'v8-history', `${projectId}.sqlite`),
      });
      for (const profile of historianProfiles) {
        historian.upsertTag({
          tagId: profile.streamId,
          name: profile.label,
          unit: profile.unit,
          source: { sourceKey: profile.sourceKey },
        });
  ...
  snapshot(projectId = this.store.getActiveProject()?.id) {
    ...
    return Object.freeze({
      ...
      historian: runtime.historian
        ? runtime.historian.status()
        : Object.freeze({ available: false, reason: configured ? 'node:sqlite unavailable in this runtime' : 'historian not configured' }),
    });
  }
}
