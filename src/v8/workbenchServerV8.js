...
const { RegisterLabService } = require('./traffic/registerLabServiceHardened');
const { mountTrafficRegisterRoutes } = require('./traffic/trafficRoutes');
const { DigitalTwinService } = require('./digitalTwin/digitalTwinServiceHardened');
const { mountDigitalTwinRoutes } = require('./digitalTwin/digitalTwinRoutes');
const { TestCenterWorkspaceService } = require('./testCenter/testCenterWorkspaceServiceHardened');
...
async function startV8ProductServer(options = {}) {
  ...
  const relay = (event) => { capture(event); broadcast({ type: 'runtime.event', event }); };
