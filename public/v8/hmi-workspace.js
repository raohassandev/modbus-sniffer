...
(() => {
  ...
  const state = { screens: [], screen: null, selectedId: null, mode: 'edit', timer: null, templates: [] };
  ...
  function renderWidget(widget) {
    ...
    if (widget.type === 'lamp') body.innerHTML = `<span class="hmi-lamp ${value === true || value === 1 ? 'on' : ''}"></span><strong>${value === true || value === 1 ? 'ON' : 'OFF'}</strong>`;
    ...
    else body.innerHTML = `<strong>${String(value)}</strong><span>${widget.binding?.unit || ''}</span>`;
  ...
  function setMode(mode) { state.mode = mode; document.querySelectorAll('[data-hmi-mode]').forEach((b)=>b.classList.toggle('primary', b.dataset.hmiMode===mode)); if (mode === 'run') startPolling(); else stopPolling(); renderCanvas(); }
  function stopPolling(){ if(state.timer) clearInterval(state.timer); state.timer=null; }
  function startPolling(){ stopPolling(); poll().catch(()=>undefined); state.timer=setInterval(()=>poll().catch(()=>undefined),1000); }
  async function poll(){ if(state.mode!=='run'||!state.screen)return; for(const widget of state.screen.widgets||[]){ if(!widget.binding?.connectionId) continue; try{ const result=await api(`/api/v8/hmi/screens/${encodeURIComponent(state.screen.screenId)}/widgets/${encodeURIComponent(widget.widgetId)}/read`,{method:'POST'}); widget._value=result.result.value; widget._history=[...(widget._history||[]).slice(-59),result.result.value]; }catch(error){ widget._value='ERR'; } } renderCanvas(); }
  ...
  $('#hmiScreenSelect')?.addEventListener('change',(e)=>{ const s=state.screens.find((x)=>x.screenId===e.target.value); if(s){state.screen=copy(s);state.selectedId=null;state.dirty=false;renderScreenList();renderCanvas();}}); $('#hmiScreenList')?.addEventListener('click',(e)=>{const b=e.target.closest('[data-hmi-screen]');if(!b)return;$('#hmiScreenSelect').value=b.dataset.hmiScreen;$('#hmiScreenSelect').dispatchEvent(new Event('change'));});
  ...
  $('#hmiCanvas')?.addEventListener('pointerdown',beginDrag); $('#hmiCanvas')?.addEventListener('click',async(e)=>{const node=e.target.closest('.hmi-widget');if(node&&state.mode==='edit'){state.selectedId=node.dataset.widgetId;renderCanvas();return;} if(state.mode!=='run'||!node)return; const widget=state.screen.widgets.find((w)=>w.widgetId===node.dataset.widgetId); try{if(e.target.closest('[data-hmi-write="switch"]')){if(!confirm(`Write ${widget.label}?`))return;await api(`/api/v8/hmi/screens/${state.screen.screenId}/widgets/${widget.widgetId}/write`,{method:'POST',body:JSON.stringify({value:!Boolean(widget._value),confirmation:{confirmed:true}})});await poll();}else if(e.target.matches('[data-hmi-write="number"]')){if(!confirm(`Write ${widget.label}?`))return;await api(`/api/v8/hmi/screens/${state.screen.screenId}/widgets/${widget.widgetId}/write`,{method:'POST',body:JSON.stringify({value:Number(e.target.value),confirmation:{confirmed:true}})});await poll();}else if(e.target.closest('[data-hmi-trigger]')){if(widget.action?.kind==='recipe'&&!confirm(`Run recipe ${widget.action.targetId}?`))return;const result=await api(`/api/v8/hmi/screens/${state.screen.screenId}/widgets/${widget.widgetId}/trigger`,{method:'POST',body:JSON.stringify({confirmation:{confirmed:true}})});if(result.result?.kind==='screen'){const target=state.screens.find((s)=>s.screenId===result.result.targetId);if(target){state.screen=copy(target);renderScreenList();renderCanvas();}}}}catch(error){toast(error.message,'error');}});
  $('#hmiSaveTemplate')?.addEventListener('click',async()=>{const w=selected();if(!w)return toast('Select a widget first','error');const name=prompt('Template name',w.label||w.type);if(!name)return;await api('/api/v8/hmi/templates',{method:'POST',body:JSON.stringify({name,widgets:[w],grid:state.screen.grid})});await refresh();toast('Template saved');}); $('#hmiApplyTemplate')?.addEventListener('click',async()=>{if(!state.screen||!$('#hmiTemplateSelect').value)return;const result=await api(`/api/v8/hmi/screens/${state.screen.screenId}/templates/${encodeURIComponent($('#hmiTemplateSelect').value)}/apply`,{method:'POST',body:JSON.stringify({x:40,y:40})});state.screen=copy(result.screen);state.dirty=false;await refresh();});
  document.querySelector('#navList')?.addEventListener('click',(event)=>{const nav=event.target.closest('[data-workspace]');if(nav?.dataset.workspace!=='hmi')return;queueMicrotask(()=>{document.querySelectorAll('.workspace').forEach((n)=>n.classList.remove('active'));$('#workspace-hmi')?.classList.add('active');refresh().catch((e)=>toast(e.message,'error'));});});
