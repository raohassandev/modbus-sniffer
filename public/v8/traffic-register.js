...
(() => {
  const ROW_HEIGHT = 36;
  const REGISTER_ROW_OVERSCAN = 10;
  ...
  function registerQuery() { const params = new URLSearchParams({limit:'10000'}); for (const [name, selector] of [['connectionId','#registerConnection'],['unitId','#registerUnit'],['area','#registerArea'],['text','#registerText']]) { const value = $(selector)?.value?.trim(); if (value) params.set(name,value); } return params; }
  ...
  function registerRowHeight(){const value=parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--row-height'));return Number.isFinite(value)&&value>0?value:46;}
  function registerSpacerRow(height){const tr=document.createElement('tr');tr.className='register-spacer-row';tr.setAttribute('aria-hidden','true');const td=document.createElement('td');td.colSpan=7;td.style.height=`${Math.max(0,height)}px`;tr.appendChild(td);return tr;}
  function registerRow(point){const tr=document.createElement('tr');tr.className=`register-row${point.sourceKey===registerState.selectedKey?' selected':''}`;tr.dataset.sourceKey=point.sourceKey;tr.tabIndex=0;tr.setAttribute('aria-selected',point.sourceKey===registerState.selectedKey?'true':'false');const engineering=point.engineering?.available?`${point.engineering.display}${point.engineering.unit?` ${point.engineering.unit}`:''}`:'—';const cells=[`${point.definition?.name||`Unit ${point.unitId}`}\n${point.connectionId}`,point.area,point.address,String(point.rawValue),engineering,point.quality,new Date(point.lastSeen).toLocaleTimeString()];cells.forEach((value,index)=>{const td=document.createElement('td');if(index===0){td.className='register-source';const lines=String(value).split('\n');td.innerHTML=`<strong>${esc(lines[0])}</strong><small>${esc(lines[1])}</small>`;}else{td.textContent=value;if([2,3,4].includes(index))td.className='register-value';if(index===4&&point.engineering?.outOfLimits)td.classList.add('out');}tr.appendChild(td);});return tr;}
  function renderRegisterRows() { const body=$('#registerBody');const viewport=$('#registerViewport');if(!body||!viewport)return;const total=registerState.points.length;$('#registerEmpty').hidden=total!==0;if(!total){body.replaceChildren();return;}const rowHeight=registerRowHeight();const visible=Math.ceil(Math.max(viewport.clientHeight,rowHeight)/rowHeight)+(REGISTER_ROW_OVERSCAN*2);const start=Math.max(0,Math.floor(viewport.scrollTop/rowHeight)-REGISTER_ROW_OVERSCAN);const end=Math.min(total,start+visible);const nodes=[];if(start>0)nodes.push(registerSpacerRow(start*rowHeight));nodes.push(...registerState.points.slice(start,end).map(registerRow));if(end<total)nodes.push(registerSpacerRow((total-end)*rowHeight));body.replaceChildren(...nodes); }
