...
(()=>{
  ...
  q('ndDrawerBody').addEventListener('click',async e=>{
    ...
    try{
      ...
      if(action==='monitor'){await api('/api/network/hosts/'+encodeURIComponent(h.id)+'/monitor',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({intervalMs:30000})});notify('Device monitoring enabled at 30 s interval');loadInventory().catch(()=>{});}
