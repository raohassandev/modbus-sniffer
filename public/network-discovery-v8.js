...
(()=>{
  ...
  root.innerHTML=`
    ...
    <section class="nd-view" data-nd-view="devices">
      <article class="panel">
        ...
        <div class="nd-monitor-strip" id="ndMonitorStrip"></div>
  ...
  async function loadInventory(){
    ...
    const [hosts,monitors]=await Promise.all([api('/api/network/hosts?'+params.toString()),api('/api/network/monitor')]);inventory=hosts;const counts={all:inventory.length,online:inventory.filter(x=>x.state==='online').length,modbus:inventory.filter(x=>x.modbus?.verified).length,industrial:inventory.filter(x=>x.industrial).length,unknown:inventory.filter(x=>x.type==='Unknown').length,monitored:monitors.length};
    q('ndInventoryFilters').innerHTML=Object.entries(counts).map(([k,v])=>`<span><b>${v}</b> ${esc(k)}</span>`).join('');
    q('ndMonitorStrip').innerHTML=monitors.length?monitors.map(m=>`<div data-monitor-id="${esc(m.hostId)}"><i class="${m.state==='online'?'online':''}"></i><div><strong>${esc(m.ip)}</strong><span>${esc(m.state||'unknown')} · ${m.lastResult?.avgRttMs==null?'RTT —':esc(m.lastResult.avgRttMs+' ms')} · loss ${esc(m.metrics?.packetLossPct??'—')}% · jitter ${m.metrics?.jitterMs==null?'—':esc(m.metrics.jitterMs+' ms')} · next ${esc(fmtTime(m.nextCheck))}</span></div><button class="secondary" data-monitor-check="${esc(m.hostId)}">Check</button><button class="danger-outline" data-monitor-stop="${esc(m.hostId)}">Stop</button></div>`).join(''):'';
  ...
  q('ndMonitorStrip').addEventListener('click',async e=>{const check=e.target.closest('[data-monitor-check]'),stop=e.target.closest('[data-monitor-stop]');try{if(check){check.disabled=true;const out=await api('/api/network/hosts/'+encodeURIComponent(check.dataset.monitorCheck)+'/monitor/check',{method:'POST'});notify(`Monitor check: ${out.state}`);}if(stop){stop.disabled=true;await api('/api/network/hosts/'+encodeURIComponent(stop.dataset.monitorStop)+'/monitor',{method:'DELETE'});notify('Device monitoring stopped');}await loadInventory();}catch(err){notify(err.message,true);}finally{if(check)check.disabled=false;if(stop)stop.disabled=false;}});

  async function openDevice(id){
    try{const host=await api('/api/network/hosts/'+encodeURIComponent(id)),events=await api('/api/network/events?hostId='+encodeURIComponent(host.id)+'&limit=100');currentDevice={...host,_history:events};renderDrawer(currentDevice);q('ndDrawer').classList.add('open');q('ndDrawerBackdrop').classList.add('open');q('ndDrawer').setAttribute('aria-hidden','false');}catch(e){notify(e.message,true);}
  }
  ...
  function renderDrawer(h){
    ...
    q('ndDrawerBody').innerHTML=`
      <div class="nd-drawer-actions"><button data-action="master" class="primary">Open in Master</button><button data-action="copy-ip" class="secondary">Copy IP</button><button data-action="copy-mac" class="secondary" ${h.mac?'':'disabled'}>Copy MAC</button><button data-action="ping" class="secondary">Ping</button><button data-action="traceroute" class="secondary">Traceroute</button><button data-action="modbus" class="secondary">Scan Modbus Units</button><button data-action="monitor" class="secondary">Monitor</button><button data-action="deep" class="secondary">Deep Scan</button><button data-action="nmap" class="secondary" ${capabilities?.nmap?.available?'':'disabled'}>Nmap Enrich</button><button data-action="snmp" class="secondary">SNMP / LLDP</button><button data-action="web" class="secondary">Open Web UI</button></div>
      <div class="nd-drawer-tabs"><button class="active" data-drawer-tab="overview">Overview</button><button data-drawer-tab="services">Services</button><button data-drawer-tab="modbus">Modbus</button><button data-drawer-tab="network">Network</button><button data-drawer-tab="history">History</button><button data-drawer-tab="evidence">Evidence</button><button data-drawer-tab="notes">Notes</button></div>
      ...
      <section class="nd-drawer-section nd-drawer-panel" data-drawer-panel="history"><h4>Device History</h4><div class="nd-events">${(h._history||[]).length?(h._history||[]).map(ev=>`<div class="nd-event"><i></i><div><strong>${esc(ev.type||'network-event')}</strong><span>${esc(ev.source||'')} ${esc(ev.severity||'')}</span><small>${esc(fmtTime(ev.at))}</small></div></div>`).join(''):'<div class="empty-state">No stored history for this device.</div>'}</div></section>
  ...
  q('ndDrawerBody').addEventListener('click',async e=>{
    ...
    try{
      ...
      if(action==='modbus'){if(!confirm(`Run read-only FC43 Unit scan on ${h.ip}? This actively transmits Modbus identification requests.`))return;b.disabled=true;const out=await api('/api/network/hosts/'+encodeURIComponent(h.id)+'/modbus',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({unitStart:1,unitEnd:247,port:h.modbus?.port||502})});currentDevice={...out.host,_history:currentDevice?._history||[]};renderDrawer(currentDevice);notify(`${out.result.responding?.length||0} responding Unit(s) found`);}
      if(action==='deep'){b.disabled=true;const out=await api('/api/network/hosts/'+encodeURIComponent(h.id)+'/deep-scan',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({timeoutMs:600,concurrency:10})});currentDevice={...out.host,_history:currentDevice?._history||[]};renderDrawer(currentDevice);notify(`Deep scan complete · ${out.summary?.openPorts||0} open port(s)`);}
      if(action==='nmap'){const allowOsDetect=confirm('Include Nmap OS fingerprinting? This sends additional active probes and may require elevated privileges. Choose Cancel for service/version enrichment only.');b.disabled=true;const out=await api('/api/network/hosts/'+encodeURIComponent(h.id)+'/nmap',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({allowOsDetect})});currentDevice={...out.host,_history:currentDevice?._history||[]};renderDrawer(currentDevice);notify(allowOsDetect?'Nmap service + OS enrichment complete':'Nmap service enrichment complete');}
      if(action==='snmp'){const community=prompt('SNMP community (used for this read-only query only):','public');if(!community)return;b.disabled=true;const out=await api('/api/network/hosts/'+encodeURIComponent(h.id)+'/snmp',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({community,readLldp:true})});currentDevice={...out.host,_history:currentDevice?._history||[]};renderDrawer(currentDevice);notify(`SNMP read complete · ${out.neighbors?.length||0} LLDP neighbor(s)`);}
      if(action==='monitor'){await api('/api/network/hosts/'+encodeURIComponent(h.id)+'/monitor',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({intervalMs:30000,pingAlways:true})});notify('Device monitoring enabled at 30 s interval with RTT/loss/jitter metrics');loadInventory().catch(()=>{});}
      if(action==='web'){const httpsSvc=(h.services||[]).find(s=>s.port===443),httpSvc=(h.services||[]).find(s=>s.port===80);const svc=httpsSvc||httpSvc;if(!svc)throw new Error('No HTTP/HTTPS service recorded for this device.');const webHost=String(h.ip||'').includes(':')?`[${h.ip}]`:h.ip;window.open(`${svc.port===443?'https':'http'}://${webHost}${[80,443].includes(svc.port)?'':':'+svc.port}`,'_blank','noopener');}
