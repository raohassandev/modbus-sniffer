...
(()=>{
  ...
  <section class="page" id="page-help"><div class="help7">
    ...
    <article class="help7-card"><h3>Start Here</h3><div class="help7-table"><table><thead><tr><th>Mode</th><th>What it does</th><th>Transmits?</th><th>First action</th></tr></thead><tbody>
      <tr><td><strong>Sniffer / Analyzer</strong></td><td>Passively learns devices, polling groups, values, timing and errors from existing Modbus traffic.</td><td>No</td><td>Settings → choose serial port/format → Apply & reconnect.</td></tr>
    ...
    <div class="help7-grid">
      <article class="help7-card"><h3>Sniffer workflow</h3><ol><li>Open <strong>Settings</strong> and select the physical serial port, baud, parity, data bits and stop bits.</li><li>Use <strong>Dashboard</strong> for overall activity/health.</li><li><strong>Devices</strong> are formed automatically from observed Unit IDs; no device project is required.</li><li><strong>Live Traffic</strong> shows request/response/timeouts and exact raw frames.</li><li><strong>Analysis</strong> checks timing, matching, CRC/LRC/MBAP, duplicates, exceptions and jitter.</li><li><strong>Registers</strong> shows learned addresses/values; <strong>Decoder/Data Lab</strong> interprets word combinations.</li></ol><p class="help7-note">Sniffer does not poll. If you need to send requests, use Master.</p></article>

      <article class="help7-card"><h3>Master workflow</h3><ol><li>Select RTU, ASCII or TCP and connect.</li><li>Enter Unit ID, FC01–04, zero-based start address and quantity.</li><li>Use <strong>Read Once</strong> first; then Start Polling.</li><li>Set retries/inter-request delay only when the device/network requires them. Serial RS-485 adapters can use RTS direction controls.</li><li>Use Monitor Sessions to save multiple polling definitions.</li><li>Use Quick Format or Data Lab for datatype/order/scaling; use Logger/Trend for history.</li><li>Writes use the guarded write panel and re-lock automatically after the operation.</li></ol><p class="help7-note">Automatic retry applies to reads/diagnostics, not guarded writes.</p></article>
