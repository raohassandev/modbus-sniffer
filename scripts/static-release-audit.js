...
const desktopClean=read('desktop/clean-dist.js');
...
const networkTopology=read('src/networkDiscovery/topology.js');
const networkMonitor=read('src/networkDiscovery/monitorManager.js');
const networkSnmp=read('src/networkDiscovery/snmpClient.js');
...
check(networkRoutes.includes("/api/network/hosts/:id/deep-scan"),'network discovery must provide a native per-device deep scan without requiring Nmap');
check(networkRoutes.includes("/api/network/scan/hosts"),'large live scans must expose paged host results');
check(networkScanManager.includes("includeHosts:false")&&networkScanManager.includes("hosts({offset=0,limit=1000}"),'large scan status updates must stay lightweight and paginate results');
check(networkMonitor.includes('packetLossPct')&&networkMonitor.includes('jitterMs'),'selected-device monitoring must retain packet-loss and jitter metrics');
check(networkTopology.includes("family:6")&&networkTopology.includes("/64"),'IPv6 hosts must participate in evidence topology through /64 grouping');
check(networkSnmp.includes("family===6?'udp6':'udp4'"),'read-only SNMP enrichment must select IPv4 or IPv6 UDP transport from the host literal');
check(networkDiscoveryUi.includes("data-action=\"deep\"")&&networkDiscoveryUi.includes("packetLossPct"),'network discovery UI must expose native deep scan and monitor quality metrics');
...
check(preflight.includes('process.env.npm_execpath')&&preflight.includes('spawnSync(process.execPath'),'Windows source preflight must invoke npm through the current Node/npm entrypoint');
check(!preflight.includes("spawnSync(process.platform==='win32'?'npm.cmd':'npm'"),'Windows source preflight must not use direct npm.cmd spawn');
check(unifiedPlaywright.includes('MODBUS_E2E_PORT')&&unifiedPlaywright.includes('reuseExistingServer: false')&&unifiedPlaywright.includes('timeout: 60000'),'unified Playwright gate must isolate its runtime port and allow bounded Windows startup time');
check(desktopClean.includes('Modbus Engineering Tool.exe')&&desktopClean.includes('taskkill')&&desktopClean.includes('maxRetries'),'Windows packaging preclean must stop stale packaged instances and retry locked dist removal');
check(desktop.scripts?.['dist:win']?.includes('node clean-dist.js'),'Windows packaging command must run locked-dist preclean before electron-builder');
